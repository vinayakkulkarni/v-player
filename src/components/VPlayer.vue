...
const { isPlaying, currentTime, duration, isBuffering, play, pause, togglePlay, seek } =
...
} = useProgressBar(progressRef, videoRef, currentTime, duration, play, pause, isPlaying);
...
const progressActive = computed(() => progressHovering.value || isSeeking.value);
      ...
      <div class="controls">
        ...
        >
          ...
          <div class="tooltip-container">
            <p class="tooltip">
              {{ formattedHoverTime }}
              <svg class="tooltip-thing" width="10" height="6" viewBox="0 0 10 6" fill="none">
          ...
          <div class="progress-bar-container">
            <div class="progress-bar" :style="{ width: `${displayPercent}%` }" />
          </div>

          <div class="bars">
            <template v-for="(segment, sIdx) in barSegments" :key="sIdx">
              <div v-for="bIdx in segment.bars" :key="`${sIdx}-${bIdx}`" class="bar" />
        ...
        <div class="time-duration">{{ formattedCurrentTime }} / {{ formattedDuration }}</div>
        ...
        <div class="volume-full-screen-container">
          <div class="volume-container" :class="{ dragging: volumeDragging }">
            ...
            >
              ...
              <div class="volume-bar-progress" :style="{ height: volumePercent }" />
            </div>
          </div>
...
.overlay {
  ...
  height: 100%;
  padding: 1rem;
  background: linear-gradient(180deg, rgba(0, 0, 0, 0.5), transparent 25% 75%, rgba(0, 0, 0, 0.5));
  pointer-events: none;
}
...
.indicator-top-container .indicator {
  position: absolute;
  bottom: 0;
  left: calc(var(--progress-hover-width, 0) + (1px - (var(--progress-hover-ratio, 0) * 1px)));
  width: 2px;
  height: 100%;
